...
void *serve_clients()
{
	...
	/*for (int i=0;i<127;i++)
	{
		stringToWriteOut_Ptr[i]='m';
	}
	stringToWriteOut_Ptr[127]='\0';*/
	...
	for(;;) {
		...
		//scanf("%s",stringToWriteOut_Ptr);
		fgets(stringToWriteOut_Ptr,200,stdin);
		char ack='0';
		printf("String is:%s",stringToWriteOut_Ptr);
		printf("Size of string: %d\n",strlen(stringToWriteOut_Ptr));
		lengthOfString=strlen(stringToWriteOut_Ptr);
